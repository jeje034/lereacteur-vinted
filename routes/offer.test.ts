...
test("GET /offers", async () => {
  await request(app)
    ...
    .expect(200)
    .then((response) => {
      expect(response.body.offers[0]._id).toBe("6081a759764c2a06a5ba5ce5");
      expect(response.body.offers[0].product_name).toBe("Tongs Roxy bleues");
      expect(response.body.offers[1]._id).toBe("6081a78b764c2a06a5ba5ce6");
...
test("PUT /offer/update with authorization, price at 6 €", async () => {
  await request(app)
    .put("/offer/update")
    .auth("lOU8e6eG0tWNMbW9E4skxlNMafepybFO1AMFZdujMFoWlREpzuqVGEZX4lwUj6vA", {
      type: "bearer",
    })
...
test("POST /offer/publish and DELETE /offer/delete", async () => {
  let offerId: string = "notInitialized";
  ...
  await request(app)
    ...
    .then(async () => {
      expect(await Offer.findById(offerId)).toBeTruthy();
    });

  //console.log("offerId: ", offerId);

  await request(app)
    .delete(`/offer/delete?id=${offerId}`)
    .auth("lOU8e6eG0tWNMbW9E4skxlNMafepybFO1AMFZdujMFoWlREpzuqVGEZX4lwUj6vA", {
      type: "bearer",
    })
    .expect(200)
    .then((response) => {
      //console.log("response,body du delete : ", response.body);
      expect(response.body).toBe("Offer removed");
    })
    .then(async () => {
      expect(await Offer.findById(offerId)).toBeFalsy();
    });
});
